...
if __name__ == "__main__":
    ...
    while True:
        ...
        try:
            ...
            # （2）client用于在kinetics_mode='complex'时，且算法无法正常计算出结果时，
            #     手动关闭matlab，解除端口绑定，若选择kinetics_mode='simple'
            # （3）traj包含全部背景车全时序的轨迹序列（注意：比赛中不允许使用此接口）
            observation, traj, client = envi.make(scenario=scenario_to_test, save_img_path=dir_save_img,
                                                  kinetics_mode='simple')  # 动力学模型选择simple参数用于比赛
