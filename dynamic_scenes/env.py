...
class Env():
    ...
    def step(self,action:Tuple[float,float],traj_future:Dict,observation_last:Observation,traj:Dict,collision_lookup:CollisionLookup) -> Observation:
        ...
        # 训练时可能不需要可视化/未来轨迹
        try:
            if traj_future is not None and observation_last is not None and traj is not None:
                self.visualizer.update(observation,traj_future,observation_last,traj)
        except Exception:
            pass
        
        return observation.format()

    def step_rl(self, action:Tuple[float,float], collision_lookup:CollisionLookup) -> Dict:
        """简化版 step：用于RL训练，跳过可视化；默认不记录磁盘以提速。"""
        observation = self.controller.step(action, collision_lookup)
        # 如需记录，可在外部将 read_only=False 再启用以下代码
        # try:
        #     self.recorder.record(observation)
        # except Exception:
        #     pass
